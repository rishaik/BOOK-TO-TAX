  ...
  return (
    <div className="dashboard-container">
      ...
      <div className="header">
        <h1 className="title">Book-Tax</h1>
        <div className="header-right">
          <select
            className="year-select mr-4"
            value={selectedYear}
      ...
      {/* Navigation Tabs */}
      <div className="nav-tabs w-full flex justify-evenly border-b border-gray-200">
        {mainTabs.map((tab) => (
          <button
            key={tab}
      ...
      {activeMainTab !== 'Diagnostics' && activeMainTab !== 'Summary View' && (
        <div className="sub-nav-tabs flex flex-wrap md:flex-nowrap space-x-8 overflow-x-auto">
          {subTabs.map((subtab) => (
            <button
              key={subtab}
        ...
        <>
          ...
          <div className="action-bar">
            <div className="search-add w-full flex justify-end">
              ...
              {isBookAdjustmentsManual ? (
                <button className="add-button ml-4 flex items-center" onClick={() => setSidePanelOpen(true)}>
                  <Plus className="add-icon" />
                </button>
              ) : (
                <button className="add-button ml-4">Import</button>
              )}
            </div>
          </div>
          ...
          <div className="table-container w-full overflow-x-auto">
            <table className="data-table min-w-[900px] w-full text-xs md:text-sm">
              <thead>
                <tr>
                  <th rowSpan={2} className="checkbox-column"></th>
